		...
		>
			{dataSearch &&
				dataSearch.data.map((row, item) => {
					...
					if (row.model === "Profile" || row.model === "Profile Extension") {
						color = "#722ed1";
					}
					let all_text = `${row.model} ${row.title} ${row.sub_content} ${row.content}`;

					if (textHighlight(all_text, search).indexOf("textHighlight") !== -1) {
						return (
								...
								<Row gutter={24}>
									<Col span={12}>
										<Tag color={color}>
											<span
												dangerouslySetInnerHTML={{
													__html: textHighlight(row.model, search),
												}}
											></span>
										</Tag>
										...
										<span
											style={{ fontSize: "10px" }}
											dangerouslySetInnerHTML={{
												__html: textHighlight(row.sub_content, search),
											}}
										></span>
									</Col>
									<Col span={12} className="text-right">
										<span
											...
											dangerouslySetInnerHTML={{
												__html: textHighlight(row.title, search),
											}}
										></span>
										...
										<span
											style={{ fontSize: "10px" }}
											dangerouslySetInnerHTML={{
												__html: textHighlight(row.content, search),
											}}
										></span>
									</Col>
								</Row>
