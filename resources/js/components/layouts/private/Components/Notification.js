...
import { Layout, Menu, Popover, Badge, Row, Col, Card, Button, notification, AutoComplete, Input, Tag } from "antd"
...
const Notification = ({ userdata, notif }) => {
  const { mutate: mutateCloseNotif, iseLoading: isLoadingCloseNotif } = useAxiosQuery("POST", "api/v1/notification/removeNotif", "user_notif")
  ...
  const { mutate: mutateIsOpen, isLoading: isLoadingIsOpen } = useAxiosQuery("POST", "api/v1/notification/isOpen", `user_notif`)
      ...
      {notif.map((item, index) => {
        ...
        return (
          <div key={index}>
            ...
            >
              <Button style={{ width: "100%", height: "70px" }} type="text">
                {/* <a
