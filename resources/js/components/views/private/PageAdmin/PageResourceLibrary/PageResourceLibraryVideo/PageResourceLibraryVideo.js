...
export default function PageResourceLibraryVideo() {
	...
	const { data: dataSource, refetch: refetchSource } = GET(
		...
		"video_resource_data_list"
	);

	const { mutate: mutateDeleteResources, isLoading: isLoadingDeleteResources } =
		DELETE("api/v1/resource", "video_resource_data_list");
			...
			onSuccess: (res) => {
				if (res.success) {
					notification.success({
						message: "Resource Library Video",
	...
	return (
		<Card id="PageResourceLibraryVideo">
			<Row gutter={[12, 12]}>
				<Col xs={24} sm={24} md={24}>
					<Button
						className="btn-main-invert b-r-none"
						onClick={() =>
							history.push("/resource-library/video-resources/add")
						}
						size="large"
						icon={<FontAwesomeIcon icon={faVideoPlus} className="m-r-sm" />}
					>
						ADD NEW VIDEO
					</Button>
				</Col>
								...
								return (
									<>
										<Button
											...
											onClick={() =>
												history.push({
													pathname: "/resource-library/video-resources/edit",
													state: record.id,
												})
											}
